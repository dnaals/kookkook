...
function Viewrecipe({dataCrl5, data5}:any) {


    const router: any = useRouter();
    const link = (name: any) => {
        let urlname = data5.filter((obj: any) => name == obj.seq);
        let url: any = urlname[0].seq;
        router.push(`/home/${url}`);
    }
    
    if (data5.length == 0) return <p className='noLike'>좋아요 레시피가 없습니다.</p>
    return (
        <>
